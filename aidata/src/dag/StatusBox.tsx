...
import { stateColors, type StateColorKey } from "src/utils/globals";
...
export const StatusWithNotes = ({
  ...
  const color =
    state && state in stateColors
      ? stateColors[state as StateColorKey]
      : "white";
  return (
    <Box
      width={boxSizePx}
...
export const SimpleStatus = ({ state, ...rest }: SimpleStatusProps) => (
  <Box
    ...
    height={boxSizePx}
    background={
      state && state in stateColors ? stateColors[state as StateColorKey] : "white"
    }
    borderRadius="2px"
    borderWidth={state ? 0 : 1}
    display="flex"
